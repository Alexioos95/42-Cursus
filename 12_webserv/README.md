...
Pour un exemple de fichier fonctionnel, regarder le fichier `config`.
...
- Les accolades ouvrante des blocks peut être sur la même ligne après plusieurs whitespaces, ou plusieurs en dessous.  
...
- Le block `error` est facultatif. S'il est présent, il ne doit y en avoir qu'un.  
- Si le block `error` est présent, il est possible qu'il soit vide. S'il ne l'est pas, chaque numero d'erreur doit obligatoirement avoir un path.  
- Le block `location` est facultatif.  
...
`error`: l'emplacement du dossier contenant les erreurs, avec le block contenant le numéro de l'erreur, et le path du fichier, relatif depuis le dossier des erreurs.  
`location`: block contenant les regles du dossier cible.
...
`dir_post`: path vers lequel redigiger les fichiers créer par un post.  
